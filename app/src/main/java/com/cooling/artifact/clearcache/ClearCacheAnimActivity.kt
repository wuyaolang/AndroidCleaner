...
import com.cooling.artifact.utils.CACHE_SCANNING_COUNT
 ...
 * 清除缓存动画页
 */
class ClearCacheAnimActivity : AppCompatActivity() {

    private var countDownTimer: CountDownTimer? = null

    companion object {
        fun newIntent(context: Context?): Intent {
            return Intent(context, ClearCacheAnimActivity::class.java)
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        countDownTimer = object : CountDownTimer(5000, 1000) {
            ...
            override fun onFinish() {
                // 保存今日扫缓次数
                var count = StorageUtils.getTodayCount(CACHE_SCANNING_COUNT)
                StorageUtils.saveTodayCount(CACHE_SCANNING_COUNT, ++count)

                binding.lavCoolingDown.cancelAnimation()
                startActivity(ClearCacheCompleteActivity.newIntent(this@ClearCacheAnimActivity))
                setResult(RESULT_OK)
                finish()
            }
        }
